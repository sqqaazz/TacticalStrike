...
ADefaultSpawningActor::ADefaultSpawningActor()
{
	...
	if (Spawning_Crystal.Succeeded())
		Spawn_Crystal = Spawning_Crystal.Class;
}
...
void ADefaultSpawningActor::SpawnUnit(FObjectInfo ObjectInfo, ADefaultBuilding* SpawningBuilding)
{
	...
	case ESpawnUnit::RifleMan:
	{
		if (SpawningBuilding->ObjectInfo.ObjectOwner == EObjectOwner::Blue)
		{
			...
			ACommanderUnit* CommanderUnit = Cast<ACommanderUnit>(SpawningBuilding->OwnerController->GetPawn());
			...
			for (float i = CrystalSpawnLocation.X; i < CrystalSpawnLocation.X + 600.0f; i += 200.0f)
			{
				for (float j = CrystalSpawnLocation.Y; j > CrystalSpawnLocation.Y - 1600.0f; j -= 200.0f)
				{
					...
					else
					{
						...
						DefaultBuilding = GetWorld()->SpawnActor<ABuilding_Crystal>(Spawn_Crystal, FVector(i, j, CrystalSpawnLocation.Z), DefaultRotator);
...
void ADefaultSpawningActor::SpawnBuilding(ESpawnBuilding BuildingType, FVector SpawnLocation)
{
	...
	case ESpawnBuilding::Barracks:
		SpawnDefaultBuilding = GetWorld()->SpawnActor<ABuilding_Barracks>(Spawn_Barracks, SpawnLocation, DefaultRotator);
		break;
	case ESpawnBuilding::SynBioLab:
		SpawnDefaultBuilding = GetWorld()->SpawnActor<ABuilding_SynBioLab>(Spawn_SynBioLab, SpawnLocation, DefaultRotator);
		break;
	case ESpawnBuilding::ResourceFacility:
		SpawnDefaultBuilding = GetWorld()->SpawnActor<ABuilding_ResourceFacility>(Spawn_ResourceFacility, SpawnLocation, DefaultRotator);
		break;
	case ESpawnBuilding::EnergyRepeater:
		SpawnDefaultBuilding = GetWorld()->SpawnActor<ABuilding_EnergyRepeater>(Spawn_EnergyRepeater, SpawnLocation, DefaultRotator);
		break;
	case ESpawnBuilding::Crystal:
		SpawnDefaultBuilding = GetWorld()->SpawnActor<ABuilding_Crystal>(Spawn_Crystal, SpawnLocation, DefaultRotator);
		break;
	default:
		SpawnDefaultBuilding = GetWorld()->SpawnActor<ABuilding_Barracks>(Spawn_Barracks, SpawnLocation, DefaultRotator);
		break;
	...
	SpawnDefaultBuilding->SetBuildingCollision();
