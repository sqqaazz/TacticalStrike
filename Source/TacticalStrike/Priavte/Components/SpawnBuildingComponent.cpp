...
void USpawnBuildingComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (TickIsActived)
	{
		...
		ACommanderController* CommanderController = Cast<ACommanderController>(OwnerController);
...
void USpawnBuildingComponent::Neutral_SpawnBuildings_Grid(FIntPoint TileGrid, ESpawnBuilding BuildingCode)
{
	...
	CurrentBuildingCode = BuildingCode;
	FVector BuildingLocation = FVector((TileGrid.X * TileSize) + (TileSize / 2), (TileGrid.Y * TileSize) + (TileSize / 2), 50.0f);
	SpawnBuildingCode(CurrentBuildingCode, EObjectOwner::None, BuildingLocation);
	GridActor->SetTile_Building(TileGrid, DefaultBuilding, false);
...
void USpawnBuildingComponent::SpawnBuildingCode(ESpawnBuilding BuildingCode, EObjectOwner ObjectOwner, FVector BuildingLocation)
{
	...
	case ESpawnBuilding::Crystal:
		Spawn_DefaultBuilding = Building_Crystal;
		DefaultBuilding = GetWorld()->SpawnActor<ABuilding_EnergyRepeater>(Spawn_DefaultBuilding, BuildingLocation, FRotator(0.0f, 0.0f, 0.0f));
		//DefaultBuilding->ObjectOwner = ObjectOwner;
		//DefaultBuilding->ObjectInfo.ObjectOwner = ObjectOwner;
		//DefaultBuilding->SetBuildingCollision();
	...
	DefaultBuilding->ObjectInfo.ObjectOwner = ObjectOwner;
	DefaultBuilding->SetBuildingCollision();
	DefaultBuilding->OwnerController = OwnerController;
